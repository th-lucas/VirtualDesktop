VirtualDesktop
==============

#### Web application using Play! framework
#### *Authors : Benoît Duriez - Maud Eyzat - Lola Giovinazzo - Thomas Lucas*

It is a "classic" Play! framework application.

Do use it, you need to have Play! framework installed on your computer. You can donwload this framework here : http://www.playframework.com/

To run the application : 

1. Start a console
2. Write the following command lines :
  - play dependencies
  - play run
3. Start your Web browser and go to : localhost:9000


The file **ManuelUtilisateur.pdf** details all the functionalities of the application. The document is written in French, but it includes a lot of screenshots which well describe the possible actions.


